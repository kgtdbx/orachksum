...
HOS sh &&orachk_debug. &&orachk_workdir./sh/call_reduce_tbl.sh &&orachk_workdir. &&orachk_version_file. &&orachk_orig_file. &&orachk_report_file. &&orachk_file_pref. &&orachk_srczip_pref. > &&orachk_dbg_file_1. 2> &&orachk_dbg_file_1. ### Generate CSV to compare.
HOS sh &&orachk_debug. &&orachk_workdir./sh/orachksum_hash.sh &&orachk_workdir. &&orachk_db_file. &&orachk_orig_file. &&orachk_csv_file. &&orachk_report_file. &&orachk_file_pref. > &&orachk_dbg_file_2. 2> &&orachk_dbg_file_2. ### Compare HASH.
HOS sh &&orachk_debug. &&orachk_workdir./sh/csv_num_rows.sh &&orachk_csv_file. &&orachk_db_file. &&orachk_csv_field_sep. &&orachk_tot_rows_file. > &&orachk_dbg_file_3. 2> &&orachk_dbg_file_3. ### Compute rows.
-- Compute rows.
...
SELECT msg,
       cnt,
       trim(to_char(round(cnt/sum(cnt) over (),4)*100,'990D99')) percent,
       NULL dummy_01
FROM   source
ORDER BY 3 DESC
]';
...
HOS sh &&orachk_debug. &&sh_csv_to_html_table. &&orachk_csv_field_sep. &&orachk_csv_file. &&one_spool_html_file. > &&orachk_dbg_file_4. 2> &&orachk_dbg_file_4.
